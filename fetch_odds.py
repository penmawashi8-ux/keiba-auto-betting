...
if odds:
print(’[fetch] OK: ’ + str(len(odds)) + ’ horses’)
break
...
```
print('[main] race_id=' + race_id)
odds_raw = fetch_odds(race_id)

result = {
    'race_id': race_id,
    'fetched_at': datetime.now(JST).isoformat(),
    'status': 'ok' if odds_raw else 'error',
    'count': len(odds_raw),
    'odds': odds_raw
}
if not odds_raw:
...
print('[main] wrote ' + str(len(odds_raw)) + ' entries to odds.json')
for o in odds_raw:
    print('  ' + str(o['horse_num']) + 'ban ' + o['horse_name'] + ' ' + str(o['odds']))
