...
def fetch_result(race_id):
    """1着馬番と確定単勝払戻倍率を返す。失敗時は(None, None)"""
    try:
        ...
        # 1着馬番:
        # <div class="Rank">1<br />着</div> の後、
        # <td class="Num WakuX"><div>枠番</div> <td class="Num WakuX"><div>馬番</div> の順
        winner = None
        m = re.search(
            r'<div class="Rank">1<br[^>]*/?>着</div>.*?'
            r'<td class="Num Waku\d+">[^<]*<div>(\d+)</div>.*?'
            r'<td class="Num Waku\d+">[^<]*<div>(\d+)</div>',
            html, re.DOTALL
        )
        if m:
            winner = int(m.group(2))  # 2番目が馬番

        # 単勝払戻: <td class="Payout"><span>250円</span>
        win_odds = None
        pay_m = re.search(
            r'class="Tansho".*?<td class="Payout"><span>([\d,]+)円</span>',
            html, re.DOTALL
        )
        if pay_m:
        ...
        if winner:
            ...
            print(f'    [FAIL] fetch_result {race_id}: 馬番パース失敗')
    except Exception as e:
...
def should_bet(spread, pick_ratio):
    # バックテスト用: ポートフォリオが組めるレースは全部記録
    # spread/ratio別の回収率はサマリーで後から分析する
    return pick_ratio > 0
    ...
    for date in dates:
        ...
        with ThreadPoolExecutor(max_workers=5) as ex:
            ...
            for future in as_completed(futures):
                ...
                if r:
                    date_rows.append(r)
                    hit_str = f'HIT +{r["payout"]-r["invest"]}円' if r['hit'] else f'MISS -{r["invest"]}円'
                    print(f'  {r["race_id"]}: spread={r["spread"]:.2f} ratio={r["pick_ratio"]:.0%} picks={r["num_picks"]} → {hit_str}')
...
def build_summary(rows):
    ...
    lines.append('')
    lines.append('--- spread閾値別 回収率 (的中=1着が購入馬に含まれる) ---')
    lines.append('')
    ...
    # 1番人気100円戦略のspread閾値別分析
    lines.append('')
    lines.append('--- 【戦略B】1番人気100円 spread閾値別 ---')
    lines.append('')
    for th in [1.0, 2.0, 3.0, 5.0, 8.0, 10.0, 15.0, 20.0]:
        ...
        lines.append(
            ...
            f'回収率={sr:6.1f}%{marker}  的中率={hr:5.1f}%  '
            f'投資={si:,}円  払戻={sp:,}円'
        )
